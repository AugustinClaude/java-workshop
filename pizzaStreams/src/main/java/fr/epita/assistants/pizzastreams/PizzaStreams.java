...
public class PizzaStreams {
    ...
    public static Double getAveragePrice(Stream<Pizza> pizzaStream) {
        return pizzaStream.mapToInt(Pizza::getPrice).average().orElse(NaN);
    }
    ...
    public static List<String> getCarnivorous(Stream<Pizza> pizzaStream) {
        return pizzaStream.filter(p -> p.getTopping().getProtein().isPresent()).map(Pizza::getName).collect(Collectors.toList());
    }
    ...
    public static List<String> getVeggies(Stream<Pizza> pizzaStream) {
        return pizzaStream.filter(p -> p.getTopping().getProtein().isEmpty() && p.getTopping().getVegetableList().size() >= 1).map(Pizza::getName).collect(Collectors.toList());
    }
    ...
    public static boolean areAllNatureItalians(Stream<Pizza> pizzaStream) {
        return pizzaStream.allMatch(p -> p.getDough().equals(Dough.CAJUN) || (p.getDough().equals(Dough.NATURE) && p.getTopping().getSauce().equals(Sauce.TOMATO) && p.getTopping().getCheese().equals(Cheese.MOZZARELLA)));
    }
}
